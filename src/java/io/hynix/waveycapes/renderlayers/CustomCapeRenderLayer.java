...
//        simulation.init(partCount);
    ...
    public void render(MatrixStack poseStack, IRenderTypeBuffer multiBufferSource, int i, AbstractClientPlayerEntity abstractClientPlayer, float f, float g, float delta, float j, float k, float l) {
        ...
        if (renderer == null || mc.player.getItemStackFromSlot(EquipmentSlotType.CHEST).getItem() == Items.ELYTRA || Unhook.unhooked) {
            return;
        }
...
//        simulation.simulate();
        ...
        if (PremiumChecker.isPremium) {
            cape = "hynix/images/capes/premiumcape.png";
        } else {
            cape = "hynix/images/capes/defaultcape.png";
        }
        if (WaveyCapesBase.config.capeStyle == CapeStyle.SMOOTH && renderer.vanillaUvValues()) {
            boolean friend = HynixMain.getInstance().getFriendManager().isFriend(abstractClientPlayer.getName().getString());
                this.renderSmoothCape(poseStack, multiBufferSource, renderer, abstractClientPlayer, delta, i, new ResourceLocation(cape), -1, -1);

//            this.renderSmoothCape(poseStack, multiBufferSource, renderer, abstractClientPlayer, delta, i, new ResourceLocation("hachclient/images/cape.png"), color2, color1);
//                this.renderSmoothCape(poseStack, multiBufferSource, renderer, abstractClientPlayer, delta, i, FastNative.getImageResource("masks/cape/logo_up.png"), Style.getMain(), Style.getSecond());
//                this.renderSmoothCape(poseStack, multiBufferSource, renderer, abstractClientPlayer, delta, i, FastNative.getImageResource("masks/cape/logo_down.png"),
//                		rock.getThemes().getCurrent() == rock.getThemes().getDarkTheme() ? FixColor.WHITE : Style.getMain(),
//                		rock.getThemes().getCurrent() == rock.getThemes().getDarkTheme() ? FixColor.WHITE : Style.getSecond());
//                this.renderSmoothCape(poseStack, multiBufferSource, renderer, abstractClientPlayer, delta, i, FastNative.getImageResource("masks/cape/logo_up.png"), color1, color2);
//                this.renderSmoothCape(poseStack, multiBufferSource, renderer, abstractClientPlayer, delta, i, FastNative.getImageResource("masks/cape/logo_down.png"),
//                		rock.getThemes().getCurrent() == rock.getThemes().getDarkTheme() ? FixColor.WHITE : color1,
//                		rock.getThemes().getCurrent() == rock.getThemes().getDarkTheme() ? FixColor.WHITE : color2);
//                this.renderSmoothCape(poseStack, multiBufferSource, renderer, abstractClientPlayer, delta, i, FastNative.getImageResource("masks/cape/qr.png"), rock.getThemes().getTextFirstColor(), rock.getThemes().getTextFirstColor());
    ...
    private void renderSmoothCape(MatrixStack poseStack, IRenderTypeBuffer multiBufferSource, CapeRenderer capeRenderer, AbstractClientPlayerEntity abstractClientPlayer, float delta, int light, ResourceLocation cape, int color1, int color2) {
    ...
    private void modifyPoseStackSimulation(final MatrixStack poseStack, final AbstractClientPlayerEntity abstractClientPlayer, final float delta, final int part) {
        ...
        poseStack.rotate(Vector3f.XP.rotationDegrees(height + naturalWindSwing));
    ...
    private static void addBackVertex(IVertexBuilder bufferBuilder, Matrix4f matrix, Matrix4f oldMatrix, float x1, float y1, float z1, float x2, float y2, float z2, int part, int light, int color1RGB, int color2RGB) {
        float i;
        Matrix4f k;
        if (x1 < x2) {
            i = x1;
        ...
        bufferBuilder.pos(oldMatrix, x1, y2, z1).color((color1RGB >> 16) & 0xFF, (color1RGB >> 8) & 0xFF, color1RGB & 0xFF, 255).tex(maxU, minV).overlay(OverlayTexture.NO_OVERLAY).lightmap(light).normal(1, 0, 0).endVertex();
        bufferBuilder.pos(oldMatrix, x2, y2, z1).color((color2RGB >> 16) & 0xFF, (color2RGB >> 8) & 0xFF, color2RGB & 0xFF, 255).tex(minU, minV).overlay(OverlayTexture.NO_OVERLAY).lightmap(light).normal(1, 0, 0).endVertex();
        bufferBuilder.pos(matrix, x2, y1, z2).color((color2RGB >> 16) & 0xFF, (color2RGB >> 8) & 0xFF, color2RGB & 0xFF, 25).tex(minU, maxV).overlay(OverlayTexture.NO_OVERLAY).lightmap(light).normal(1, 0, 0).endVertex();
        bufferBuilder.pos(matrix, x1, y1, z2).color((color1RGB >> 16) & 0xFF, (color1RGB >> 8) & 0xFF, color1RGB & 0xFF, 255).tex(maxU, maxV).overlay(OverlayTexture.NO_OVERLAY).lightmap(light).normal(1, 0, 0).endVertex();
    }

    private static void addFrontVertex(IVertexBuilder bufferBuilder, Matrix4f matrix, Matrix4f oldMatrix, float x1, float y1, float z1, float x2, float y2, float z2, int part, int light, int color1RGB, int color2RGB) {
        float i;
        Matrix4f k;
        if (x1 < x2) {
            i = x1;
        ...
        bufferBuilder.pos(oldMatrix, x1, y1, z1).color((color1RGB >> 16) & 0xFF, (color1RGB >> 8) & 0xFF, color1RGB & 0xFF, 255).tex(maxU, maxV).overlay(OverlayTexture.NO_OVERLAY).lightmap(light).normal(1, 0, 0).endVertex();
        bufferBuilder.pos(oldMatrix, x2, y1, z1).color((color2RGB >> 16) & 0xFF, (color2RGB >> 8) & 0xFF, color2RGB & 0xFF, 255).tex(minU, maxV).overlay(OverlayTexture.NO_OVERLAY).lightmap(light).normal(1, 0, 0).endVertex();
        bufferBuilder.pos(matrix, x2, y2, z2).color((color2RGB >> 16) & 0xFF, (color2RGB >> 8) & 0xFF, color2RGB & 0xFF, 255).tex(minU, minV).overlay(OverlayTexture.NO_OVERLAY).lightmap(light).normal(1, 0, 0).endVertex();
        bufferBuilder.pos(matrix, x1, y2, z2).color((color1RGB >> 16) & 0xFF, (color1RGB >> 8) & 0xFF, color1RGB & 0xFF, 255).tex(maxU, minV).overlay(OverlayTexture.NO_OVERLAY).lightmap(light).normal(1, 0, 0).endVertex();
    }

    private static void addLeftVertex(IVertexBuilder bufferBuilder, Matrix4f matrix, Matrix4f oldMatrix, float x1, float y1, float z1, float x2, float y2, float z2, int part, int light, int color1RGB, int color2RGB) {
        float i;
        if (x1 < x2) {
            i = x1;
            x1 = x2;
        ...
        bufferBuilder.pos(matrix, x2, y1, z1).color((color1RGB >> 16) & 0xFF, (color1RGB >> 8) & 0xFF, color1RGB & 0xFF, 255).tex(maxU, maxV).overlay(OverlayTexture.NO_OVERLAY).lightmap(light).normal(1, 0, 0).endVertex();
        bufferBuilder.pos(matrix, x2, y1, z2).color((color1RGB >> 16) & 0xFF, (color1RGB >> 8) & 0xFF, color1RGB & 0xFF, 255).tex(minU, maxV).overlay(OverlayTexture.NO_OVERLAY).lightmap(light).normal(1, 0, 0).endVertex();
        bufferBuilder.pos(oldMatrix, x2, y2, z2).color((color1RGB >> 16) & 0xFF, (color1RGB >> 8) & 0xFF, color1RGB & 0xFF, 255).tex(minU, minV).overlay(OverlayTexture.NO_OVERLAY).lightmap(light).normal(1, 0, 0).endVertex();
        bufferBuilder.pos(oldMatrix, x2, y2, z1).color((color1RGB >> 16) & 0xFF, (color1RGB >> 8) & 0xFF, color1RGB & 0xFF, 255).tex(maxU, minV).overlay(OverlayTexture.NO_OVERLAY).lightmap(light).normal(1, 0, 0).endVertex();
    }

    private static void addRightVertex(IVertexBuilder bufferBuilder, Matrix4f matrix, Matrix4f oldMatrix, float x1, float y1, float z1, float x2, float y2, float z2, int part, int light, int color1RGB, int color2RGB) {
        float i;
        if (x1 < x2) {
            i = x1;
            x1 = x2;
        ...
        bufferBuilder.pos(matrix, x2, y1, z2).color((color2RGB >> 16) & 0xFF, (color2RGB >> 8) & 0xFF, color2RGB & 0xFF, 255).tex(minU, maxV).overlay(OverlayTexture.NO_OVERLAY).lightmap(light).normal(1, 0, 0).endVertex();
        bufferBuilder.pos(matrix, x2, y1, z1).color((color2RGB >> 16) & 0xFF, (color2RGB >> 8) & 0xFF, color2RGB & 0xFF, 255).tex(maxU, maxV).overlay(OverlayTexture.NO_OVERLAY).lightmap(light).normal(1, 0, 0).endVertex();
        bufferBuilder.pos(oldMatrix, x2, y2, z1).color((color2RGB >> 16) & 0xFF, (color2RGB >> 8) & 0xFF, color2RGB & 0xFF, 255).tex(maxU, minV).overlay(OverlayTexture.NO_OVERLAY).lightmap(light).normal(1, 0, 0).endVertex();
        bufferBuilder.pos(oldMatrix, x2, y2, z2).color((color2RGB >> 16) & 0xFF, (color2RGB >> 8) & 0xFF, color2RGB & 0xFF, 255).tex(minU, minV).overlay(OverlayTexture.NO_OVERLAY).lightmap(light).normal(1, 0, 0).endVertex();
    }

    private static void addBottomVertex(IVertexBuilder bufferBuilder, Matrix4f matrix, Matrix4f oldMatrix, float x1, float y1, float z1, float x2, float y2, float z2, int part, int light, int color1RGB, int color2RGB) {
        float i;
        if (x1 < x2) {
            i = x1;
            x1 = x2;
        ...
        bufferBuilder.pos(oldMatrix, x1, y2, z2).color((color1RGB >> 16) & 0xFF, (color1RGB >> 8) & 0xFF, color1RGB & 0xFF, 255).tex(maxU, minV).overlay(OverlayTexture.NO_OVERLAY).lightmap(light).normal(1, 0, 0).endVertex();
        bufferBuilder.pos(oldMatrix, x2, y2, z2).color((color2RGB >> 16) & 0xFF, (color2RGB >> 8) & 0xFF, color2RGB & 0xFF, 255).tex(minU, minV).overlay(OverlayTexture.NO_OVERLAY).lightmap(light).normal(1, 0, 0).endVertex();
        bufferBuilder.pos(matrix, x2, y1, z1).color((color2RGB >> 16) & 0xFF, (color2RGB >> 8) & 0xFF, color2RGB & 0xFF, 255).tex(minU, maxV).overlay(OverlayTexture.NO_OVERLAY).lightmap(light).normal(1, 0, 0).endVertex();
        bufferBuilder.pos(matrix, x1, y1, z1).color((color1RGB >> 16) & 0xFF, (color1RGB >> 8) & 0xFF, color1RGB & 0xFF, 255).tex(maxU, maxV).overlay(OverlayTexture.NO_OVERLAY).lightmap(light).normal(1, 0, 0).endVertex();
    }

    private static void addTopVertex(IVertexBuilder bufferBuilder, Matrix4f matrix, Matrix4f oldMatrix, float x1, float y1, float z1, float x2, float y2, float z2, int part, int light, int color1RGB, int color2RGB) {
        float i;
        if (x1 < x2) {
            i = x1;
            x1 = x2;
...
//        int color1RGB = color1();
//        int color2RGB = color2.getRGB();

        bufferBuilder.pos(oldMatrix, x1, y2, z1).color((color1RGB >> 16) & 0xFF, (color1RGB >> 8) & 0xFF, color1RGB & 0xFF, 255).tex(maxU, maxV).overlay(OverlayTexture.NO_OVERLAY).lightmap(light).normal(0, 1, 0).endVertex();
        bufferBuilder.pos(oldMatrix, x2, y2, z1).color((color2RGB >> 16) & 0xFF, (color2RGB >> 8) & 0xFF, color2RGB & 0xFF, 255).tex(minU, maxV).overlay(OverlayTexture.NO_OVERLAY).lightmap(light).normal(0, 1, 0).endVertex();
        bufferBuilder.pos(matrix, x2, y1, z2).color((color2RGB >> 16) & 0xFF, (color2RGB >> 8) & 0xFF, color2RGB & 0xFF, 255).tex(minU, minV).overlay(OverlayTexture.NO_OVERLAY).lightmap(light).normal(0, 1, 0).endVertex();
        bufferBuilder.pos(matrix, x1, y1, z2).color((color1RGB >> 16) & 0xFF, (color1RGB >> 8) & 0xFF, color1RGB & 0xFF, 255).tex(maxU, minV).overlay(OverlayTexture.NO_OVERLAY).lightmap(light).normal(0, 1, 0).endVertex();
    }
    ...
    private CapeRenderer getCapeRenderer(AbstractClientPlayerEntity abstractClientPlayer, IRenderTypeBuffer multiBufferSource) {
        ...
        if (PremiumChecker.isPremium) {
            ...
            CustomCapeRenderLayer.vanillaCape.vertexConsumer = multiBufferSource.getBuffer(RenderType.getEntityCutout(new ResourceLocation(cape)));
