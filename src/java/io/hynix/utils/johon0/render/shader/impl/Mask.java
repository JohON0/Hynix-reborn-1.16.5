package io.hynix.utils.johon0.render.shader.impl;
...
public class Mask implements IMinecraft {
    ...
    public void renderMask(float x, float y, float width, float height, Runnable mask) {
        ...
        ShaderUtils.mask.attach();
        
        ShaderUtils.mask.setUniformf("location", (float) (x * mc.getMainWindow().getGuiScaleFactor()),
                (float) ((mc.getMainWindow().getHeight() - (height * mc.getMainWindow().getGuiScaleFactor()))
                        - (y * mc.getMainWindow().getGuiScaleFactor())));
        ShaderUtils.mask.setUniformf("rectSize", width * mc.getMainWindow().getGuiScaleFactor(),
