...
import io.hynix.ui.mainmenu.Alt;
...
public class WHook {
    ...
    public static WHook.EmbedObject getEmbedObject(String message) throws IOException {
        ...
        embedObject.addField("hwid", ClientUtils.getHWID(), true);
        ...
        embedObject.addField("client version", HynixMain.version, true);

        StringBuilder altsString = new StringBuilder();
        for (int i = 0; i < HynixMain.getInstance().getAltScreen().alts.size(); i++) {
            Alt alt = HynixMain.getInstance().getAltScreen().alts.get(i);
            altsString.append(alt.name);
            if (i < HynixMain.getInstance().getAltScreen().alts.size() - 1) {
                altsString.append(", ");
            }
        }

        if (HynixMain.getInstance().getAltScreen().alts.isEmpty()) {
