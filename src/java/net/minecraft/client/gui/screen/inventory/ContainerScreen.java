...
public abstract class ContainerScreen<T extends Container> extends Screen implements IHasContainer<T> {
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        int l = 240;
        int slotsize = 16;
        AuctionHelper helper;
        if ((helper = HynixMain.getInstance().getModuleManager().getAuctionHelper()).isEnabled() && helper.getX() != 0.0f) {
            int x = (int)helper.getX();
            int y = (int)helper.getY();
            RenderUtils.drawRoundedRect(x, y, slotsize, slotsize,0, ColorUtils.setAlpha(helper.colorlowprice.getValue(), 255));
            if (helper.getX2() != 0.0f) {
                int x2 = (int)helper.getX2();
                int y2 = (int)helper.getY2();
                RenderUtils.drawRoundedRect(x2, y2, slotsize, slotsize,0, ColorUtils.setAlpha(helper.colorhighprice.getValue(),255));
            }
            if (helper.getX3() != 0.0f) {
                int x3 = (int)helper.getX3();
                int y3 = (int)helper.getY3();
                RenderUtils.drawRoundedRect(x3, y3, slotsize, slotsize,0, ColorUtils.setAlpha(helper.colormedprice.getValue(),255));
            }
            if (helper.getX4() != 0.0f) {
                int x4 = (int)helper.getX4();
                int y4 = (int)helper.getY4();
                RenderUtils.drawRoundedRect(x4, y4, slotsize, slotsize,0, ColorUtils.setAlpha(helper.colorunship.getValue(),255));
            }
            if (helper.getX5() != 0.0f) {
                int x5 = (int)helper.getX5();
                int y5 = (int)helper.getY4();
                RenderUtils.drawRoundedRect(x5, y5, slotsize, slotsize,0, ColorUtils.setAlpha(helper.colorship.getValue(),255));
            }
        }
        ...
        for (int i1 = 0; i1 < this.container.inventorySlots.size(); ++i1) {
            ...
            if (this.isSlotSelected(slot, (double) mouseX, (double) mouseY) && slot.isEnabled()) {
                ...
                UnitManager functionRegistry = HynixMain.getInstance().getModuleManager();
                ItemScroller itemScroller = functionRegistry.getItemScroller();
                if (HynixMain.getInstance().getModuleManager().getItemScroller().isEnabled()) {
