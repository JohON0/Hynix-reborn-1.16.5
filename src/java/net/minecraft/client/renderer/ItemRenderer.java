...
public class ItemRenderer implements IResourceManagerReloadListener {
    ...
    public void renderItemIntoGUI2(ItemStack stack, int x, int y) {
        float itemWidth = 10.0F; // Ширина предмета
        float itemHeight = 16.0F; // Высота предмета
        this.renderItemModelIntoGUI2(stack, x, y, itemWidth, itemHeight, this.getItemModelWithOverrides(stack, (World) null, (LivingEntity) null));
    }

    protected void renderItemModelIntoGUI2(ItemStack stack, int x, int y, float itemWidth, float itemHeight, IBakedModel bakedmodel) {
        renderItemGui = true;
        ...
        RenderSystem.translatef(itemWidth / 2.0F, itemHeight / 2.0F, 0.0F); // Центрируем предмет
        RenderSystem.scalef(1.0F, -1.0F, 1.0F);
        RenderSystem.scalef(itemWidth / 16.0F, itemHeight / 16.0F, 16.0F); // Изменяем масштаб по ширине и высоте
