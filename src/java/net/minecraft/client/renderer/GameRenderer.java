...
public class GameRenderer implements IResourceManagerReloadListener, AutoCloseable {
    ...
    private void hurtCameraEffect(MatrixStack matrixStackIn, float partialTicks) {
        ...
        HynixMain.getInstance().getEventBus().post(eventCancelOverlay);

        if (!HynixMain.getInstance().getModuleManager().getSelfDestruct().unhooked) {
    ...
    private void applyBobbing(MatrixStack matrixStackIn, float partialTicks) {
        if (this.mc.getRenderViewEntity() instanceof PlayerEntity) {
            if (Unhook.unhooked || mc.player.isElytraFlying() || mc.player.fallDistance > 2) {
            ...
            } else if (!Unhook.unhooked || mc.player.isOnGround() || mc.gameSettings.keyBindJump.isPressed()) {
    ...
    public void renderHand(MatrixStack p_renderHand_1_, ActiveRenderInfo p_renderHand_2_, float p_renderHand_3_,
        ...
        UnitManager moduleManager = HynixMain.getInstance().getModuleManager();
        CustomViewItem glassHand = moduleManager.getViewModel();
    ...
    public Matrix4f getProjectionMatrix(ActiveRenderInfo activeRenderInfoIn, float partialTicks, boolean useFovSetting) {
        ...
        if(HynixMain.getInstance().getModuleManager().getAspectRatio().isEnabled() && !Unhook.unhooked)
            aspectRatio = AspectRatio.value.getValue().floatValue();
        matrixstack.getLast().getMatrix().mul(Matrix4f.perspective(this.getFOVModifier(activeRenderInfoIn, partialTicks, useFovSetting), aspectRatio, 0.05F, this.clipDistance));
