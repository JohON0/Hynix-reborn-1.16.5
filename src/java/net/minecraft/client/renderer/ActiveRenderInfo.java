...
import io.hynix.events.impl.EventCamera;
    ...
    private double calcCameraDistance(double startingDistance) {
        ...
        boolean isBetter = betterMinecraft.isEnabled() && betterMinecraft.smoothCamera.getValue();
         for (int i = 0; i < 8; ++i) {
             ...
             if  (!Unhook.unhooked && isBetter) {
                 interpolatedDistance = MathUtils.fast((float) interpolatedDistance, (float) startingDistance, 10);
