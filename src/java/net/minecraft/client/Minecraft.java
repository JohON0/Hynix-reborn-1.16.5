...
public class Minecraft extends RecursiveEventLoop<Runnable> implements ISnooperInfo, IWindowEventListener {
    ...
    public void setDefaultMinecraftTitle() {
        ...
        if (selfDestruct.unhooked) {
            ...
            this.mainWindow.setWindowTitle(HynixMain.name + " #" + HynixMain.version + " " + HynixMain.site);
        }
    }
    ...
    private void tick(boolean isDebug, @Nullable LongTickDetector detector) {
        HynixMain.getInstance().getEventBus().post(eventTick);
                ...
                for (Hand hand : Hand.values()) {
                    ...
                    if (this.objectMouseOver != null) {
                        switch (this.objectMouseOver.getType()) {
                            case ENTITY -> {
                                UnitManager moduleManager = HynixMain.getInstance().getModuleManager();
                                AttackAura hitAura = moduleManager.getAttackAura();
                                ...
                                boolean hitAuraIsStateAndTargetNotNull = hitAura.isEnabled() && hitAura.target != null;
    ...
    public void runTick() {
        ...
        if (this.world != null) {
            HynixMain.getInstance().getEventBus().post(player.tick);
    ...
    public void loadWorld(ClientWorld worldClientIn) {
        ...
        HynixMain.getInstance().getEventBus().post(new EventChangeWorld());
    }
    ...
    public boolean isEntityGlowing(Entity entity) {
        return entity.isGlowing() || this.player != null && this.player.isSpectator() && this.gameSettings.keyBindSpectatorOutlines.isKeyDown() && entity.getType() == EntityType.PLAYER && !HynixMain.getInstance().getModuleManager().getNoRender().isEnabled() && !HynixMain.getInstance().getModuleManager().getNoRender().element.is("Эффект свечения").getValue();
    }
