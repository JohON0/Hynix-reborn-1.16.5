...
import io.hynix.events.impl.EventMouseButtonPress;
import io.hynix.events.impl.EventRotate;
...
public class MouseHelper {
    ...
    private void mouseButtonCallback(long handle, int button, int action, int mods) {
        if (handle == this.minecraft.getMainWindow().getHandle()) {
            ...
            if (!aboolean[0] && (this.minecraft.currentScreen == null || this.minecraft.currentScreen.passEvents)
                ...
                if (flag) {
                    HynixMain.getInstance().onKeyPressed(-100 + i);

                    HynixMain.getInstance().getEventBus().post(new EventMouseButtonPress(i));
    ...
    public void updatePlayerLook() {
        ...
        if (this.isMouseGrabbed() && this.minecraft.isGameFocused()) {
            ...
            FreeCamera freeCamera = HynixMain.getInstance().getModuleManager().getFreeCam();

            this.minecraft.getTutorial().onMouseMove(d2, d3);

            if (freeCamera.isEnabled()) {
                freeCamera.getFakePlayer().rotateTowards(d2, d3 * (double) i);
            } else if (this.minecraft.player != null) {
                eventRotate.setYaw(d2);
                eventRotate.setPitch(d3 * (double) i);

                HynixMain.getInstance().getEventBus().post(eventRotate);
