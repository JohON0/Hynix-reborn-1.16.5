...
import io.hynix.events.impl.EventDamageReceive;
...
public abstract class LivingEntity extends Entity {
    ...
    public boolean onLivingFall(float distance, float damageMultiplier) {
        ...
        if (i > 0) {
            ...
            if (this instanceof ClientPlayerEntity) {
                HynixMain.getInstance().getEventBus()
    ...
    private int getArmSwingAnimationEnd() {
        ...
        CustomViewItem animation = moduleManager.getViewModel();

        if (animation.isEnabled() && animation.swingAnim.getValue() && this instanceof ClientPlayerEntity) {
            return 25 - ((animation.animationMode.is("Flip")) ? (animation.swingSpeed.getValue().intValue()) : (animation.swingSpeed.getValue().intValue() * 2));
        }
    ...
    protected void jump() {
        ...
        if (this instanceof ClientPlayerEntity entity) {
            HynixMain.getInstance().getEventBus().post(eventJump);
        }
    ...
    public void travel(Vector3d travelVector) {
        if (this.isServerWorld() || this.canPassengerSteer()) {
            ...
            if (this.isInWater() && this.func_241208_cS_() && !this.func_230285_a_(fluidstate.getFluid())) {
                ...
                AutoCrystal autoExplosion = HynixMain.getInstance().getModuleManager().getAutoExplosion();
                boolean hitAuraIsNull = hitAura.target == null || !hitAura.isEnabled();

                // hitAura

                if (hitAura.isEnabled() && hitAura.options.is("Коррекция движения").getValue() && this instanceof ClientPlayerEntity) {
                    vector3d1 = this.getVectorForRotation(hitAura.rotateVector.y, hitAura.rotateVector.x);
                }
                float f = this.rotationPitch * ((float) Math.PI / 180F);
                if (hitAura.isEnabled() && hitAura.options.is("Коррекция движения").getValue() && this instanceof ClientPlayerEntity) {
                    f = hitAura.rotateVector.y * ((float) Math.PI / 180F);
                }
    ...
    public Vector3d getPositon(float partialTicks) {
        return MathUtils.interpolate(getPositionVec(), new Vector3d(lastTickPosX, lastTickPosY, lastTickPosZ), partialTicks);
    }
